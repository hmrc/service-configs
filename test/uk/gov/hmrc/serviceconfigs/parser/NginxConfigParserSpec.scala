...
class NginxConfigParserSpec
  ...
  "NginxConfigParser" should {
    "parse regex routes" in {
      ...
      val cfg = new NginxConfigParser(nginxConfig).parseConfig(configRegex).getOrElse(sys.error(s"Expected to parse $configRegex"))
    ...
    "parse normal routes" in {
      ...
      val cfg = new NginxConfigParser(nginxConfig).parseConfig(configNormal).getOrElse(sys.error(s"Expected to parse $configNormal"))
    ...
    "drop routes without a proxy_pass" in {
      ...
      val cfg = new NginxConfigParser(nginxConfig).parseConfig(config).getOrElse(sys.error(s"Expected to parse $config"))
