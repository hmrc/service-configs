...
case class DeploymentConfigGrouped(
                                serviceName: ServiceName,
                                configs: Seq[DeploymentEnvironment]
                               )

object DeploymentConfigGrouped {
  private implicit val snf = ServiceName.format

  implicit val format: Format[DeploymentConfigGrouped] =
    ((__ \ "_id").format[ServiceName]
      ~ (__ \ "configs").format[Seq[DeploymentEnvironment]]
      )(DeploymentConfigGrouped.apply, unlift(DeploymentConfigGrouped.unapply))
