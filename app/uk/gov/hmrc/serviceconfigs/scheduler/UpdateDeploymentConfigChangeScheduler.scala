...
class UpdateDeploymentConfigChangeScheduler @Inject()(
  ...
  scheduleWithTimePeriodLock(
    ...
    logger.info("Updating deployment config change data")
          ...
          .foldLeftM(()): (_, d) =>
            ...
            for
              pag <- releasesApiConnector
                      .deploymentHistory(env, now.minus(90, ChronoUnit.DAYS), now, d.serviceName)
              ...
              _   <- pag.history.foldLeftM(()):
                      case (_, deployment) =>
                        ...
                        yield ()
            yield ()
      .map(_ => logger.info("Finished updating deployment config change data"))
