                                 ...
                                 case Some(true) =>
                                   ...
                                   configService.configByEnvironment(serviceName, Seq(env), version = None, latest = false)
                                     .map: newDeployedConfig =>
                                       ...
                                       logger.debug(s"Detected a change in configId $serviceName ${deployment.version} in $env (${previousDeploymentInfo.map(_.configId)} -> ${deployment.configId}) - did config actually change? $changed")
                                       Some(changed)
                                 case other =>
