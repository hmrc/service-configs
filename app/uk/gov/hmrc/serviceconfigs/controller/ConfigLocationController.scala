    ...
    Action.async:
      for
        appConfigBase  <- appConfigService
                            ...
                            .map: x =>
                              Map(s"app-config-base" -> x.map(_ => s"https://github.com/hmrc/app-config-base/blob/HEAD/${serviceName.asString}.conf"))
        appConfigEnv   <- Environment
                            .values
                            .toList
                            .foldLeftM[Future, Map[String, Option[String]]](Map.empty):
                              (acc, env) => appConfigService
        ...
        upscan         <- upscanConfigRepository
                            ...
                            .map:
                              _.map(u => (s"upscan-config-${u.environment.asString}" -> Some(u.location)))
                               .toMap
        outagePages    <- outagePageRepository
                            .findByServiceName(serviceName)
                            .map:
                              _.getOrElse(Nil).map(env => (s"outage-page-${env.asString}" -> Some(s"https://github.com/hmrc/outage-pages/blob/main/${env.asString}")))
                               .toMap
