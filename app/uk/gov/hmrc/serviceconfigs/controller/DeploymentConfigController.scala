...
class DeploymentConfigController @Inject()(
  ...
  def deploymentConfigV2(serviceName: Option[ServiceName], teamName: Option[TeamName], sort: Option[String]): Action[AnyContent] =
    Action.async {
      ...
      val getReposByTeam = tn match {
        case Some(value) => teamsAndRepositoriesConnector.getRepos(teamName = Some(value)).map(Option(_))
        case None => Future.successful(None)
      }

      for {
        teamRepos   <- getReposByTeam
        configs     <- deploymentConfigService.findAll(sn, teamRepos, sort)
      } yield  {
