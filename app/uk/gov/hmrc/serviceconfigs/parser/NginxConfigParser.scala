...
class NginxConfigParser @Inject()(
  ...
  object NginxTokenParser extends Parsers {
    ...
    def extractKillSwitch(ifblocks: List[IFBLOCK]): Option[ShutterSwitch] =
      ifblocks
        .filter(_.predicate.contains(shutterConfig.shutterKillswitchPath))
        .map(ib => extractShutterSwitch(shutterConfig.shutterKillswitchPath, ib))
        .headOption

    private val p = s"""\\(-f(${shutterConfig.shutterServiceSwitchPathPrefix.trim}[a-zA-Z0-9_-]+)\\)""".r
    ...
    def parameter1: Parser[PARAM] =
      (keyword ~ rep1(value) <~ SEMICOLON()) ^^ {
    ...
    def parameter2: Parser[PARAM] =
      (keyword ~ keyword ~ rep(value) <~ SEMICOLON()) ^^ {
        case kw ~ p1 ~ List(v) => OTHER_PARAM(kw.keyword, p1.keyword ++ v.value)
      }

    def paramaterViaComment: Parser[PARAM] =
      comment ^^ {
        case c if c.comment.startsWith("#!") => MARKER_COMMENT(c.comment.stripPrefix("#!").takeWhile(_ != ' '))
        case _ => COMMENT_LINE()
      }

    def parameter: Parser[PARAM] =
      parameter1 | parameter2 | paramaterViaComment

    def block: Parser[List[NGINX_AST]] =
      rep1(parameter | context)

    def context: Parser[NGINX_AST] =
      (keyword ~ rep(value) ~ OPEN_BRACKET() ~ block ~ CLOSE_BRACKET()) ^^ {
...
object NginxLexer extends RegexParsers {
  ...
  def semicolon: Parser[SEMICOLON] =
    ";" ^^ { _ => SEMICOLON() }
  ...
  def url: Parser[VALUE] =
    """https?:\/\/(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,6}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)""".r ^^ (k => VALUE(k.trim))
  ...
  def value: Parser[VALUE] =
    url | quotedValue | unquotedValue

  def keyword: Parser[KEYWORD] =
    """[a-zA-Z_][a-zA-Z0-9_]* """.r ^^ (k => KEYWORD(k.trim))
